    ...
    <div className="p-6 animate-fade-in-up">
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div
            className="relative z-10 w-full max-w-lg rounded-2xl border border-white/8 p-6 shadow-2xl"
            style={{ background: "rgba(10,14,26,0.97)", backdropFilter: "blur(24px)" }}
          >
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-white/90">Neuer A/B Test</h2>
              <button onClick={() => setShowCreate(false)} className="text-white/35 hover:text-white/70 transition-colors">
                <X className="h-5 w-5" />
              </button>
            </div>

            {error && (
              <div className="mb-4 rounded-xl border border-rose-500/25 bg-rose-500/10 px-4 py-3 text-sm text-rose-400">
                {error}
              </div>
            )}
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-white/50 mb-1.5">Testname</label>
                <input
                  name="name"
                  required
              ...
              <div>
                <label className="block text-xs font-medium text-white/50 mb-1.5">Projekt</label>
                <select
                  name="project_id"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-medium text-white/50 mb-1.5">Prompt A</label>
                ...
                <div>
                  <label className="block text-xs font-medium text-white/50 mb-1.5">Prompt B</label>
              ...
              <div>
                ...
                <div className="flex justify-between mt-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs text-indigo-400 font-semibold">A</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-purple-400 font-semibold">B</span>
      ...
      {localTests.length === 0 ? (
        <div
          className="flex flex-col items-center justify-center rounded-2xl border border-white/7 py-20 text-center"
        ...
        >
          ...
          <p className="font-semibold text-white/70 mb-1">Noch keine A/B Tests</p>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="font-semibold text-white/90">{test.name}</span>
                    <span
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    ...
                    >
                      <div className="text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-1">Variant A &middot; {test.weight_a}%</div>
                    ...
                    >
                      <div className="text-[10px] font-bold text-purple-400 uppercase tracking-wider mb-1">Variant B &middot; {test.weight_b}%</div>
                      <div className="text-sm text-white/70 font-mono truncate">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    disabled={isPending}
                    className="text-white/35 hover:text-indigo-400 transition-colors"
      ...
      >
        ...
        <span className="text-white/40">
          A/B-Tests sind ein <span className="font-semibold text-indigo-400">Pro-Feature</span>. Upgraden, um unbegrenzte Tests zu erstellen.
        </span>
      </div>
    </div>
  );
